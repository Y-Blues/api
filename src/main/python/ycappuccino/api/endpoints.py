...
class UrlPath(object):
    ...
    def __init__(self, a_method, a_url, a_api_description):
        ...
        for w_path in a_api_description._body["paths"].keys():
            w_path_pattern = (
                re.sub(r"\{.*\}", ".*", w_path).replace("/", r"\/").replace("$", r"\$")
                + "$"
            )
...
class EndpointResponseModel(EndpointResponse):
    ...
    def get_json(self):
        if self._meta is None:
            ...
            if self._body is not None:
                if isinstance(self._body, dict):
                    w_resp["data"] = self._body.get(self._storage_property, self._body)
                elif isinstance(self._body, list):
                    w_resp["data"] = [
                        (
                            w_model[self._storage_property]
                            if isinstance(w_model, dict)
                            and self._storage_property in w_model
                            else w_model
                        )
...
def get_token_from_header(a_headers):
    if "authorization" in a_headers:
        ...
        for w_cookie in a_headers["Cookie"].split(";"):
            w_name, w_separator, w_value = w_cookie.strip().partition("=")
            if w_separator and "_ycappuccino" in w_name:
                return w_value
    return None
